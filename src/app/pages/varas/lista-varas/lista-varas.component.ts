...
export class ListaVarasComponent implements OnInit, AfterViewInit {
  ...
  noRecords = true;
  state = 'state-varas';
  nameColumns = 'varasColumns';

  constructor(
    private title: Title,
    private varasService: VarasService,
    private errorHandler: ErrorHandlerService,
    private filtroVara: FiltroVarasService,
  ...
  ngOnInit() {
    ...
    if (!localStorage.getItem('varasColumns')) {
      this.setColumnsDefaultValue();
    } else {
      // get selected columns from local storage
      this.selectedColumns = JSON.parse(localStorage.getItem('varasColumns'));
    }
  }
  ...
  applySearch(value: any) {
    ...
    if (
      ...
      this.filtroVara.filtro(value, this.filtro).then((obj) => {
        this.filtro = obj;
        this.saveLocalStorage(null);
        this.carregar();
        this.FirstPage();
