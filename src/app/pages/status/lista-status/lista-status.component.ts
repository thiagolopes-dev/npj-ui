...
export class ListaStatusComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private filtroStatus: FiltroStatusService,
  ...
  ngOnInit() {
    ...
    if (!localStorage.getItem('statusColumns')) {
      ...
      this.selectedColumns = JSON.parse(localStorage.getItem('statusColumns'));
    }
  }
  ...
  applySearch(value: any) {
    ...
    if (
      ...
      this.filtroStatus.filtro(value, this.filtro).then((obj) => {
        this.filtro = obj;
        this.saveLocalStorage(null);
        this.carregar();
        this.FirstPage();
